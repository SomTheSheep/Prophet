...
TIME_START = SIM_CONFIG.get(
    "LIVE_START",
    "2026-04-13T17:55:09Z"
)

TIME_END = SIM_CONFIG.get(
    "LIVE_END",
    "2026-04-20T13:27:15Z"
)

TRAIN_WINDOW_DAYS = float(
    SIM_CONFIG.get("TRAIN_WINDOW_DAYS", 12)
)

RETRAIN_EVERY_HOURS = float(
    SIM_CONFIG.get("RETRAIN_EVERY_HOURS", 6)
)
...
def apply_prophet_univariate(df_target, forecast_points, metric_cfg=None):

    if metric_cfg is None:
        metric_cfg = {}

    df_merged = df_target.sort_values('ds').ffill().fillna(0)

    prophet_cfg = metric_cfg.get('prophet', {})

    interval_width = float(
        prophet_cfg.get(
            'interval_width',
            os.environ.get('PROPHET_INTERVAL_WIDTH', 0.80)
        )
    )
    ...
    changepoint_prior_scale = float(
        prophet_cfg.get(
            ...
            os.environ.get('PROPHET_CHANGEPOINT_PRIOR_SCALE', 0.05)
        )
    )
    ...
    seasonality_prior_scale = float(
        prophet_cfg.get(
            ...
            os.environ.get('PROPHET_SEASONALITY_PRIOR_SCALE', 15.0)
        )
    )

    fourier_order_daily = int(
        prophet_cfg.get('fourier_order_daily', 6)
    )

    add_subdaily = bool(
        prophet_cfg.get('add_subdaily', True)
    )

    fourier_order_subdaily = int(
        prophet_cfg.get('fourier_order_subdaily', 3)
    )
    ...
    m.add_seasonality(
        name='daily',
        period=1,
        fourier_order=fourier_order_daily
    )
    ...
    if add_subdaily:
        m.add_seasonality(
            name='hourly',
            period=1/24,
            fourier_order=fourier_order_subdaily
        )
...
def simulate_historical_data():
    ...
    logger.info(
        f"Starting Prophetig backfill simulator "
    ...
    while virtual_now < end_dt:
        ...
        logger.info(
            f"--- Processing Virtual Time Window: {virtual_now} ---"
        )

        train_start = (
            virtual_now - timedelta(days=TRAIN_WINDOW_DAYS)
        ).timestamp()

        train_end = virtual_now.timestamp()

        next_virtual_now = (
            virtual_now + timedelta(hours=RETRAIN_EVERY_HOURS)
        )
        ...
        for metric_name, queries in METRICS_CONFIG.items():
            ...
            logger.info(
                f"[{metric_name}] "
                f"Training {TRAIN_WINDOW_DAYS}d history"
            )
            ...
            if df_target_full.empty:
                logger.warning(
                    f"[{metric_name}] No target data found."
                )
                continue
            ...
            forecast, m = apply_prophet_univariate(
                df_target_train,
                forecast_points=int(forecast_pts),
                metric_cfg=queries
            )
        ...
        if results_df_list:
            ...
            logger.info(
                f"Writing {len(master_df)} rows to PostgreSQL"
            )
